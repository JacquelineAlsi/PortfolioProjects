...
SELECT 
  column_name
  , COUNT(table_name) AS column_name_count 
...
SELECT 
  Id
  , (TotalTimeInBed - TotalMinutesAsleep) As minutes_to_fall_asleep 
FROM `portfolioprojects2023.bellabeat.SleepDay`  
ORDER BY 1; 
...
SELECT 
  Id
  , ROUND(MIN(TotalMinutesAsleep/60)) As min_hours_asleep
...
SELECT 
  ...
  , SUM(SedentaryMinutes + LightlyActiveMinutes + FairlyActiveMinutes + VeryActiveMinutes) AS total_minutes 
FROM `portfolioprojects2023.bellabeat.DailyActivity`     
GROUP BY 1,2,3; 
...
SELECT 
  activity.ActivityDate AS day_of_week   
 , ROUND(AVG(SedentaryMinutes)) AS sedentary_minutes    
 , ROUND(AVG(LightlyActiveMinutes)) AS lightly_active_minutes     
 , ROUND(AVG(FairlyActiveMinutes)) AS fairly_active_minutes     
 , ROUND(AVG(VeryActiveMinutes)) AS very_active_minutes    
...
SELECT 
 activity.Id, ROUND(AVG(SedentaryMinutes)) AS sedentary_minutes   
 , ROUND(AVG(LightlyActiveMinutes)) AS lightly_active_minutes   
 , ROUND(AVG(FairlyActiveMinutes)) AS fairly_active_minutes   
 , ROUND(AVG(VeryActiveMinutes)) AS very_active_minutes  
 , ROUND(AVG(TotalMinutesAsleep/60)) As hours_asleep      
...
SELECT 
 Id
 , ROUND(AVG(LightActiveDistance),2) AS light_active_dist 
